...
def train(class_tag, model, train_data_set, save, n_epochs=3,
    ...
    with open('./data_cache/protein_to_aa_map.pkl', "rb") as fp:
    ...
    train_loader = torch.utils.data.DataLoader(train_data_set, batch_size=batch_size,
                                               ...
                                               num_workers=5, drop_last=False, collate_fn=graph_collate)
    ...
    # Start log
    with open(os.path.join(save, 'DeepPPI_results_{}.csv'.format(num)), 'w') as f:
        ...
        for epoch in range(n_epochs):
            ...
            if f_max > best_F:
                ...
                torch.save(model.state_dict(), os.path.join(save, 'DeepPPI_model_{}.dat'.format(num)))
            else:
                count += 1
                if count >= 5:
                    return None
