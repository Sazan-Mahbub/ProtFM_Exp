...
from models.exp_ppi import ExpModel
...
#             print(output.__len__(), output.shape)
    ...
    return acc, f_max, p_max, r_max,auc, aupr,mcc,t_max
...
def predict(model_file,test_data,window_size,path_dir,ratio):
    ...
    model = model.cuda()
    # print(test_list.__len__(), test_list[0])
    results = test(model, test_loader,path_dir)
...
if __name__ == '__main__':
    ...
    window_size = 3
    path_dir = "./checkpoints/saved_models"
    ...
    results_all = []
    for saved_model_number in [1,2,3,4]:
        model_file = "{0}/model_{1}.dat".format(path_dir, saved_model_number)
        results = predict(model_file,datas,window_size,path_dir,ratio_list)
        results_all += [results]
    
    results_all = np.array(results_all)
    
    print(f'Average over {results_all.shape[0]} runs')
    print(print(
        'acc:%0.6f,F_value:%0.6f, precision:%0.6f,recall:%0.6f,auc:%0.6f,aupr:%0.6f,mcc:%0.6f,threadhold:%0.6f\n' % results_all.mean(0).tolist())
