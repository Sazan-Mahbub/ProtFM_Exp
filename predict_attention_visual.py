...
from models.deep_ppi_attention_visual import DeepPPI
from generator import data_generator_attention_visual as data_generator
from generator.data_generator_attention_visual import graph_collate
...
def test(model, loader,path_dir,pre_num=1):
    ...
    for batch_idx, (protbert_data, local_data, label, label_idx_onehot, graph_batch, protein_info) in enumerate(loader):
        # Create vaiables
        ...
        with torch.no_grad():
            ...
            # compute output
            t0 = time.time()
            output, head_attn_scores = model(protbert_var, local_var, label_idx_onehot, graph_batch)
            print(output.__len__(), output.shape)
            shapes = output.data.shape
            output = output.view(shapes[0], 500)
            output = output.data.cpu().numpy()
            # result.append(output.data.cpu().numpy())
            # attention_scores.append(head_attn_scores)
            # print(head_attn_scores[0].shape, head_attn_scores[0].view(-1).shape, head_attn_scores[0].view(-1, 500, 20, 1).shape, len(head_attn_scores))
            head_attn_scores = head_attn_scores[0].view(shapes[0], 500, 20).numpy()
            # print(head_attn_scores.shape)
            # print('graph_batch edges:', graph_batch.edges()[0].shape)
            graph_list = dgl.unbatch(graph_batch)
            # print('graph_list:', len(graph_list))
            for i, graph in enumerate(graph_list):
                __len_limit__ = min(500, protein_info[i]['seq_length'])
                all_trues.append(label[i])
                ...
                # print(i, protein_infos_[-1], edges[-1].shape, attention_scores[-1].shape, result[-1].shape, all_trues[-1].shape)
                # print(edges[-1])
            # exit()
    ...
    result_file = "{0}/prediction_and_attention_scores.pkl".format(path_dir)
    ...
    for x in result:
        result_ += x.tolist()
    all_trues = np.array(all_trues_)
    all_preds = np.array(result_)
...
def predict(model_file,test_data,window_size,path_dir,ratio):
    ...
    test_list_file = 'data_cache/test_considering_only_proteins.pkl'
    # parameters
    ...
    print(test_list.__len__(), test_list)
