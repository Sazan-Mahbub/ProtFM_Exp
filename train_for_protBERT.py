...
def train_epoch(model, loader, optimizer, epoch, all_epochs, print_freq=100):
  ...
  for batch_idx, (protbert_data, local_data, label, label_idx_onehot) in enumerate(loader):
    # Create vaiables
    with torch.no_grad():
      if torch.cuda.is_available():
        protbert_var = torch.autograd.Variable(protbert_data.cuda(async=True).float())
        ...
        label_idx_onehot = torch.autograd.Variable(label_idx_onehot.cuda(async=True).float())
      else:
    ...
    # compute output
    output = model(protbert_var, local_var, label_idx_onehot)
