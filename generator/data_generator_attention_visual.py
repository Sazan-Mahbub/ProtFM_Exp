...
class dataSet(data.Dataset):
    def __init__(self, window_size, protBert_file=None, label_file=None, protein_list_file=None):
        ...
        """
                All mean and variance for benchmark_dataset, calculated from trainset
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(_all_protBert_feature_).type(torch.FloatTensor), \
               ...
               G, \
               protein_info # torch.from_numpy(np.array([src, dst])).long()\
    ...
    def add_edges_custom(self, G, neighborhood_indices, edge_features):
        ...
        # print(G.num_edges(), edge_features.shape)
        # print(G.edata['ex'].shape, G.edata['ex'].max(), G.edata['ex'].min(), G.edata['ex'].mean(), np.array(temp_edge_features).shape, G)

        # print(threading.get_ident(), '1>',time()-t1)
        # raise
        # for nbr in neighborhood_indices[center]:
        #     if center != nbr:
        #         G.add_edge(nbr, center)  # (src: nbr[0], dst: center)
        #         # src += [nbr]
        #         # dst += [center]
        #         # G.edata['ex'][nbr, center] = edge_features[nbr, center]
        #     else:
        #         issues_in_nbr_data.append([center, nbr, 'self_loop'])
        #         print('>> self_loop:', center, nbr)
        #         raise
        # return src, dst
...
def graph_collate(samples):
    ...
    #  (graph, label).
    protbert_data, local_data, label, label_idx_onehot, graph_batch, protein_info = map(list, zip(*samples))
    ...
    return protbert_data, local_data, label, label_idx_onehot, graph_batch, protein_info
